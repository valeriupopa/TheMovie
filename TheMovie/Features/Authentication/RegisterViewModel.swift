...
class RegisterViewModel {
    ...
    init(data: RegisterViewDataModel) {
        ...
        let inputsObservable = Observable.combineLatest(passwordObservable, data.nameObservable, data.emailObservable, birthDayObservable) { (password: String, name: String?, email: String?, birthday: Date) -> (String, String, String, Date) in
            return (password, name ?? "", email ?? "", birthday)
        }
